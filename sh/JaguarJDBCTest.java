...
public class JaguarJDBCTest 
{
    ...
    public void testJaguarDataSource( String ip, String ports ) throws SQLException 
	{
		...
		//q = "insert into jbench values ( 'n12\nline\\'2', 'v12\nval\"ue' ) ; ";
		//statement.executeUpdate( q );
		...
		while(rs.next()) {
			...
			n = rs.getInt(1);
			System.out.println( "id: " + key + " phone: " + addr + " ***** qwer n=" + n  );
		}
		...
		rs = statement.executeQuery("select id, phone from asctable where id>10;");
		while(rs.next()) {
			key = rs.getString("id");
			addr = rs.getString("phone");
    ...
    public void testJaguarDataSourcePrepare( String ip, String ports ) throws SQLException 
	{
		...
		System.out.println( q );
		// connection.prepareStatement(q).executeUpdate(q);
		...
		// bind
		q = "select id, phone from asctable where id < ? and ( phone > 232 or phone > ? ) and id < 100 ";
		System.out.println( q );
		ps = connection.prepareStatement( q );
		ps.setInt(1, 1001 );
		ps.setInt(2, 103 );
